...
from models.gcn_ensemble import GCN
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save-path', default='save/gcn-ensemble')
    ...
    n = len(wnids)
    edge = graph['edges']
    edge = [tuple(e) for e in edge]
    edge += [(v, u) for u, v in edge]
    edge += [(u, u) for u in range(n)]
    edges.append(edge)
    ...
    gcn = GCN(n, edges, weights, word_vectors, word_vectors.shape[1], fc_vectors.shape[1], hidden_layers).cuda()
