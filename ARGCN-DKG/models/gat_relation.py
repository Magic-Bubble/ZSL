...
class SpGraphAttentionLayer(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, dropout=False, relu=True):
        ...
        self.a = nn.Parameter(torch.empty(in_channels * 2, 1))
    ...
    def forward(self, inputs, adj):
        ...
        edge_e = torch.cat([inputs[edge[0, :], :], inputs[edge[1, :], :]], dim=1)
        ...
        h_prime = self.special_spmm(edge, edge_e, torch.Size([N, N]), inputs)
        assert not torch.isnan(h_prime).any()

        h_prime = h_prime.div(e_rowsum)
        assert not torch.isnan(h_prime).any()

        outputs = torch.mm(torch.cat([h_prime, inputs], dim=1), self.w) + self.b
