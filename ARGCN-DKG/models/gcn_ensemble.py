...
class GraphConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, dropout=False, relu=True):
        ...
        self.w = nn.Parameter(torch.empty(in_channels * 2, out_channels))
    ...
    def forward(self, inputs, adj):
        ...
        inputs1 = torch.mm(adj[0], inputs)
        inputs2 = torch.mm(adj[1], inputs)
        inputs = torch.cat([inputs1, inputs2], dim=1)
        outputs = torch.mm(inputs, self.w) + self.b
...
class GCN(nn.Module):
    ...
    def __init__(self, n, edges, weights, word_vectors, in_channels, out_channels, hidden_layers):
        super().__init__()
       
        edges = [np.array(edge) for edge in edges]

        uniform_adj = [self.get_adj(np.ones(len(edge)), edge, n) for edge in edges]
